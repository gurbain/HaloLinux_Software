...
//	std::cout << "Disparity Min, Max: [" << param.disp_min << ", " << param.disp_max << "]\n";
	...
	if (subsample) {
		...
		cv::Mat disparityImage_float(cv::Size(dims[0]/2,dims[1]/2),CV_32F,D1_data,cv::Mat::AUTO_STEP);
		...
		cv::Mat disparityImage_float(cv::Size(dims[0],dims[1]),CV_32F,D1_data,cv::Mat::AUTO_STEP);
...
cv::Mat DenseStereo::calculate(const cv::Mat & leftImage, const cv::Mat & rightImage) {
	...
	for (int i=0; i<disparityImage.cols; i++) {
		for (int j=0; j<disparityImage.rows; j++) {
			...
			double d = disparityImage.at<uint8_t>(j, i);
