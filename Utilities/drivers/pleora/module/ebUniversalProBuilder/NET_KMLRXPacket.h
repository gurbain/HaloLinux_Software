...
/// \return Result state
///
OS_EXTERN OS_RESULT NET_RXPacketInit( OS_DRV_CONTEXT* aContext, NET_RXPACKET* aPacket, OS_VOID* aData, OS_UINT32 aSize, 
...
///
OS_EXTERN OS_VOID NET_RXPacketDestroy( NET_RXPACKET* aPacket, OS_UINT32 aDprLevel );
...
#define NET_RXPACKETGETUDPPORT( aPacket )                                       \
    ( ( aPacket )->UDP->DestinationPort )

/// 
/// \brief Copy function of a Ipv4 packet
/// 
/// \param aPacket Pre-Allocated Packer Manager
/// \param aOffset Offset after the UDP header to skip
/// \param aDest Memory destination ( should be at least aSize bytes )
/// \param aSize Size in bytes to be copied
///
#define NET_RXPACKETCOPY( aPacket, aOffset, aDest, aSize )                      \
    NET_Copy( ( aPacket )->Buffer, ( aOffset )                                  \
        + ( aPacket )->HeaderOffsetForCopy                                      \
        + sizeof( NET_HEADER_UDP ), ( aDest ), ( aSize ) )
...
#define NET_RXPACKETCOMPARE( aPacket, aLocalInfo, aRemoteInfo )                                                                             \
    ( ( ( aPacket )->UDP->DestinationPort == ( aLocalInfo )->Port )                                                                         \
        && ( (     ( ( aPacket )->Ethernet->EthType == NET_ETH_TYPE_IPV4 )                                                                  \
