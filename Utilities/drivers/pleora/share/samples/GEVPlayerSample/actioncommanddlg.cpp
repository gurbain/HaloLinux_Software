...
ActionCommandDlg::ActionCommandDlg( GEVPlayer *aGEVPlayer )
    : QDialog( aGEVPlayer )
	, mForceACKFlag( false )
{
	CreateLayout();
...
void ActionCommandDlg::CreateLayout()
{
	...
	// Acknowledges group box

	QVBoxLayout *lAckVLayout = new QVBoxLayout;

	mAcknowledgesEdit = new QTextEdit;
	mAcknowledgesEdit->setReadOnly( true );
	lAckVLayout->addWidget( mAcknowledgesEdit );
...
void ActionCommandDlg::FillNetworkList()
{
    ...
    PvUInt32 lCount = mActionCommand.GetInterfaceCount();
...
bool ActionCommandDlg::Configure()
{
	...
	// Device key
	PvUInt64 lDeviceKey = 0;
	if ( EditToInt( mDeviceKeyEdit, std::numeric_limits<PvUInt32>::max(), lDeviceKey ) )
	{
	...
	// Group key
	PvUInt64 lGroupKey = 0;
	if ( EditToInt( mGroupKeyEdit, std::numeric_limits<PvUInt32>::max(), lGroupKey ) )
	{
	...
	// Group mask
	PvUInt64 lGroupMask = 0;
	if ( EditToInt( mGroupMaskEdit, std::numeric_limits<PvUInt32>::max(), lGroupMask ) )
	{
	...
	if ( lScheduled )
	{
		PvUInt64 lScheduledTime = 0;
		if ( EditToInt( mTimestampEdit, std::numeric_limits<PvUInt64>::max(), lScheduledTime ) )
		{
...
///
/// \brief Monitors the acknowledges after sending an action command
///

void ActionCommandDlg::MonitorAcknowledges()
{
	QString lAcks;

	PvUInt32 lCount = mActionCommand.GetAcknowledgeCount();
	for ( PvUInt32 i = 0; i < lCount; i++ )
	{
	...
	mAcknowledgesEdit->setText( lAcks );
}
...
bool ActionCommandDlg::EditToInt( QLineEdit *aEdit, PvUInt64 aMax, PvUInt64 &aValue )
{
	bool lOK = false;
