...
void GEVPlayer::Connect( PvDeviceInfo *aDI, PvConfigurationReader *aCR )
{
    ...
    if ( lDI == NULL )
    {
        ...
        {
            QString lDeviceIPAddressString;
            lDeviceIPAddressString.sprintf( "%lld.%lld.%lld.%lld",
            ...
            lSystem.FindDevice( lDeviceIPAddressString.toAscii().data(), &lDI );
        }
    }

    if ( lDI != NULL )
    {
        mManufacturerLine->setText( lDI->GetVendor().GetAscii() );
        mModelLine->setText( lDI->GetModel().GetAscii() );
        mIpAddressLine->setText( lDI->GetIPAddress().GetAscii() );
        mMacAddressLine->setText( lDI->GetMACAddress().GetAscii() );
        mNameLine->setText( lDI->GetUserDefinedName().GetAscii() );
    }
...
void GEVPlayer::Disconnect()
{
    ...
    mTerminalIPEngineWnd->SetDevice( NULL );
...
void GEVPlayer::OnImageDisplayedEvent()
{
    ...
    else
    {
    	...
    	QString lStatistics = lInfo.GetStatistics( mDisplayThread->GetFPS() ).GetAscii();
...
void GEVPlayer::OnFileMenuAboutToShow()
{
    ...
    mFileSaveAction->setEnabled( ( mDeviceConnected || mStream->IsOpen() ) && !lStreaming );
    mFileSaveAsAction->setEnabled( ( mDeviceConnected || mStream->IsOpen() ) && !lStreaming );
    mFileOpenAction->setEnabled( ( !mCommunicationWnd->GetQWidget()->isVisible() ) &&
            ( !mDeviceWnd->GetQWidget()->isVisible() ) &&
            ( !mStreamWnd->GetQWidget()->isVisible() ) && !lStreaming );
...
void GEVPlayer::OnToolsMenuAboutToShow()
{
    ...
    mToolsDisplayOptionsAction->setEnabled( mDisplayThread->IsRunning() );
    mToolsBufferOptionsAction->setEnabled( mStream->IsOpen() );

    mToolsSaveImagesAction->setEnabled( mDisplayThread->IsRunning() );
    mToolsSaveCurrentImageAction->setEnabled( mDisplayThread->IsRunning() );
    ...
    mToolsSerialAction->setEnabled( true );

    mToolsSaveXMLAction->setEnabled( mDevice.IsConnected() );

    mToolsDeinterlacingDisabled->setEnabled( mDisplayThread->IsRunning() );
	mToolsDeinterlacingDisabled->setChecked( false );
    mToolsDeinterlacingWeaving->setEnabled( mDisplayThread->IsRunning() );
	mToolsDeinterlacingWeaving->setChecked( false );

    switch ( mDisplayThread->GetDeinterlacing() )
    {
    case PvDeinterlacingDisabled:
    	mToolsDeinterlacingDisabled->setChecked( true );
    	break;

    case PvDeinterlacingWeaving:
...
void GEVPlayer::OpenConfig( QString aFileName )
{
    ...
    PvPropertyList lDisplayOptions;
    lReader.Restore( TAG_DISPLAYOPTIONS, lDisplayOptions );
    mDisplayThread->Load( lDisplayOptions );

    // Whether you are connected or not you should be able to restore the current buffer options
...
void GEVPlayer::OnToolsActionCommandAction()
{
...
void GEVPlayer::OnToolsDisplayOptionsAction()
{
    ...
    lDlg.Init( mDisplayThread );
    if ( lDlg.exec() == QDialog::Accepted )
    {
        lDlg.Apply( mDisplayThread );
    }
}
...
void GEVPlayer::SaveConfig( QString aLocation, bool aSaveConnectedDevice )
{
    ...
    PvPropertyList lDisplayOptions;
    mDisplayThread->Save( lDisplayOptions );
    lWriter.Store( &lDisplayOptions, TAG_DISPLAYOPTIONS );
...
{
    SendEventIfPossible( WM_ACQUISITIONSTATECHANGED );
}
...
///
/// \brief Messaging channel event with associated GenICam parameters handler
///

void GEVPlayer::OnEventGenICam( PvDevice *aDevice, PvUInt16 aEventID, PvUInt16 aChannel, PvUInt64 aBlockID,
    PvUInt64 aTimestamp, PvGenParameterList *aData )
{
    ...
    if ( mLogBuffer->IsEventsEnabled() )
    {
        // Basic event log
        QString lLog;
        lLog.sprintf( "Event ID: 0x%X Channel: 0x%X BlockID: 0x%X, Timestamp: 0x%X",
        ...
        if ( aData != NULL )
        {
            ...
            while ( lP != NULL )
            {
                QString lName = lP->GetName().GetAscii();
                if ( lName.indexOf( "DataLength" ) >= 0 )
                {
...
void GEVPlayer::SendEventIfPossible( int aMsg )
{
