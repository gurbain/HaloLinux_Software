...
void PvDualSourceSample::Connect( PvDeviceInfo *aDeviceInfo )
{
	...
	if ( lSourceSelector != NULL )
	{
		for ( PvUInt16 i = 0; i < 2; i++ )
		{
			...
			lResult = lSources[i]->Open( &mDevice,
					aDeviceInfo->GetIPAddress(), aDeviceInfo->GetInterface()->GetIPAddress(),
					static_cast<PvUInt16>( lChannel ) );
			if ( !lResult.IsOK() )
			{
				PvMessageBox( this, lResult );
				return;
	...
	else
	{
		...
		lResult = mSource1->Open( &mDevice,
				aDeviceInfo->GetIPAddress(), aDeviceInfo->GetInterface()->GetIPAddress(), 0 );
		if ( !lResult.IsOK() )
		{
			PvMessageBox( this, lResult );
			return;
