...
//////     Open     //////
//////////////////////////
int ORF::init(bool auto_exposure, int integration_time, int modulation_freq, int amp_threshold, string ether_addr, string img_directory, string calib_filename) 
{
	// Assign calib_filename variable
	this->calib_filename = calib_filename;
	...
	DEBUG<<"SwissRanger device open. Number of images available: "<<inr_<<endl;
	if ( (cols_ != ORF_COLS) || (rows_ != ORF_ROWS) || (inr_ < ORF_IMAGES) || (imgEntryArray_ == 0) ) {
		SafeCleanup();
		DEBUG<<"Invalid data images: "<<inr_<<" "<<cols_<<" x"<<rows_<<" images received from camera! Expected "<<ORF_COLS<<"x"<<ORF_ROWS<<" images."<<endl;
		return (-1);
	}
...
//////   Read Data  //////
//////////////////////////
int ORF::capture(Mat& depthNewImageFrame, Mat& visualNewImageFrame, Mat& confidenceNewImageFrame, TimeStamp& ts)
{
	// Start the timeStamp
	ts.start();
...
int ORF::intrinsicCalib()
{
	...
	FileStorage storage(this->calib_filename, FileStorage::WRITE);
	storage<<"Intrinsicparameters"<<intrinsicMatrix;
	storage<<"Distortioncoefficients"<<distortionCoeffs;
	storage.release();
	
	// Print saving info
	INFO<<"Calibration matrixes has been saved in "<<this->calib_filename<<endl;
	
	return 0;
}

int ORF::captureRectified(Mat& depthNewImageFrame, Mat& visualNewImageFrame, Mat& confidenceNewImageFrame, TimeStamp& ts)
{
	// Start the timeStamp
	ts.start();
...
int ORF::saveRectified()
{
	...
	// Capture images
	this->captureRectified(depthNewImageFrame, visualNewImageFrame, confidenceNewImageFrame, ts);
	...
	string type = ".png";
	ssd<<this->img_directory<<"/"<<named<<imgNum<<type;
	ssv<<this->img_directory<<"/"<<namev<<imgNum<<type;
	ssc<<this->img_directory<<"/"<<namec<<imgNum<<type;
	...
	if (imgNum==0)
		INFO<<"Saving ORF images into folder "<<this->img_directory<<endl;
