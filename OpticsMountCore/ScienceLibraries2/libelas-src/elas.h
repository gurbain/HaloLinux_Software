...
public:
  ...
  // parameter settings
  struct parameters {
    int32_t disp_min;               // min disparity	(0,0)
    int32_t disp_max;               // max disparity	(255,255)
    float   support_threshold;      // max. uniqueness ratio (best vs. second best support match)	(0.85, 0.95)
    int32_t support_texture;        // min texture for support points	(10,10)
    int32_t candidate_stepsize;     // step size of regular grid on which support points are matched	(5,5)
    int32_t incon_window_size;      // window size of inconsistent support point check	(5,5)
    int32_t incon_threshold;        // disparity similarity threshold for support point to be considered consistent	(5,5)
    int32_t incon_min_support;      // minimum number of consistent support points	(5,5)
    bool    add_corners;            // add support points at image corners with nearest neighbor disparities	(0,1)
    int32_t grid_size;              // size of neighborhood for additional support point extrapolation		(20,20)
    float   beta;                   // image likelihood parameter	(0.02,0.02
    float   gamma;                  // prior constant	(3,5)
    float   sigma;                  // prior sigma		(1,1)
    float   sradius;                // prior sigma radius	(2,3)
    int32_t match_texture;          // min texture for dense matching	(1,0)
    int32_t lr_threshold;           // disparity threshold for left/right consistency check		(2,2)
    float   speckle_sim_threshold;  // similarity threshold for speckle segmentation	(1,1)
    int32_t speckle_size;           // maximal size of a speckle (small speckles get removed)	(200,200)
    int32_t ipol_gap_width;         // interpolate small gaps (left<->right, top<->bottom)		(3,5000)
    bool    filter_median;          // optional median filter (approximated)		(0,1)
    bool    filter_adaptive_mean;   // optional adaptive mean filter (approximated)		(1,0)
    bool    postprocess_only_left;  // saves time by not postprocessing the right image		(1,0)
    bool    subsampling;            // saves time by only computing disparities for each 2nd pixel	(0,0)
  ...
  bool getExceptionOccurred() { return exception_occurred;}
