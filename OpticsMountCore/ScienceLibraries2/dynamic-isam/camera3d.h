...
class cameraPose3d {
  friend std::ostream& operator<<(std::ostream& out, const cameraPose3d& p) {
    p.write(out);
    return out;
  }
...
public:
  ...
  cameraPose3d(double x, double y, double z, double yaw, double pitch, double roll) : _t(x, y, z), _rot(yaw, pitch, roll) {}
  ...
  cameraPose3d exmap(const Eigen::Vector3d& delta) {
