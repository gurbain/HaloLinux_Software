...
char serialib::Open(const char *Device,const unsigned int Bauds){
 ...
 char serialib::WriteString(const void *String,long unsigned int NbBytes)
 {
 #if defined (_WIN32) || defined( _WIN64)
     DWORD dwBytesWritten;                                               // Number of bytes written
     if(!WriteFile(hSerial,String,NbBytes,&dwBytesWritten,NULL))  // Write the string
         return -1;                                                      // Error while writing
     return 1;                                                           // Write operation successfull
 #endif
 #ifdef __linux__
      if (write(fd,String,NbBytes)!=NbBytes)                                // Write the string
         return -1;                                                      // error while writing
     return 1;                                                           // Write operation successfull
 #endif
 }
